...
def test_query_base(listings_collection_one_room, sample_room):
    """
    ...
    body = response["body"]
    assert body == [sample_room]


def test_query_with_area(listings_collection, sample_rooms):
    ...
    context = {}
    response = lambda_handler(event, context)
    expected_rooms = [room for room in sample_rooms if room["neighborhood"] == area]
    body = response["body"]
...
def test_query_with_check_in_check_out(listings_collection, sample_rooms):
    ...
    expected_rooms = [
        ...
        if check_in <= room["calendar"][0]["date"] <= check_out
    ]
    body = response["body"]
